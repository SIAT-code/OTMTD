...
cost_routines = {
    1: (lambda x, y: geomloss.utils.distances(x, y)), # sqrt of squared_distances
    2: (lambda x, y: geomloss.utils.squared_distances(x, y) / 2), # "D_xx - 2 * D_xy + D_yy" (already devided by p=2)
}
...
class DatasetDistance():
    ...
    def _load_infer_labels(self, D, classes=None, reindex=None, reindex_start=None):
        """ Load and infer labels via unsupervised Clustering (KMeans/DBSCAN).
        """
        if classes:
            k = len(classes)
    ...
    def _init_data(self, D1, D2):
        ...
        targets1, classes1, idxs1 = extract_data_targets(D1) # Wrapper around extract_dataloader_targets and extract_dataset_targets
...
class FeatureCost():
    ...
    def __call__(self, X1, X2):
        ...
        if self.src_emb is not None:
            ...
            try: # view to (B1, N1, W, H) for Callable model infering, W*H=D1, N1 is the channels of image
                X1 = self.src_emb(X1.view(-1,*self.src_dim).to(self.device)).reshape(B1, N1, -1) # (B1, N1, W*H) (B1, 3, 28*28)
            except: # Memory error?
        ...
        if self.p == 1:
            c = geomloss.utils.distances(X1, X2) # sqrt of squared_distances
        elif self.p == 2:
            c = geomloss.utils.squared_distances(X1, X2) / 2 # "D_xx - 2 * D_xy + D_yy", shape: (B, N1, N2)
        else:
            raise ValueError()

...
class SelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # 将一层中的多个head的attention保存
        single_layer_attn = attention
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, trg, trg_mask=None):
        ...
        trg_, attn = self.sa(trg, trg, trg, trg_mask)
        trg = self.ln(trg + self.do(trg_))
...
class SeqStrucFusion(nn.Module):
    ...
    def forward(self, seq_feat, struc_feat, protein_mask):
        ...
        for layer in self.layers:
            multimodal_emb, single_layer_attn = layer(multimodal_emb, protein_mask)
            all_layers_attn.append(single_layer_attn)
        
        multimodal_emb = self.ln(multimodal_emb)

        return multimodal_emb, all_layers_attn
...
class GotermEncoder(nn.Module):
    ...
    def forward(self, goterms, goterms_mask):
        ...
        for layer in self.layers:
            goterms, _ = layer(goterms, goterms_mask)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, trg, src, trg_mask=None, src_mask=None):
        ...
        trg_, _ = self.sa(trg, trg, trg, trg_mask)
        trg = self.ln(trg + self.do(trg_))
        
        trg_, attn  = self.ea(trg, src, src, src_mask)
        trg = self.ln(trg + self.do(trg_))
        trg = self.ln(trg + self.do(self.pf(trg)))
...
class ProGoCrossFusion(nn.Module):
    ...
    def forward(self, pro_feat, go_feat, pro_mask=None, go_mask=None):
        all_pro_layers_attn, all_go_layers_attn = [], []
        ...
        for layer in self.pro_layers:
            ...
            pro_feat_new, single_pro_layer_attn = layer(pro_feat, go_feat_for_pro, pro_mask, go_mask)
            all_pro_layers_attn.append(single_pro_layer_attn)
        ...
        for layer in self.go_layers:
            ...
            go_feat_new, single_go_layer_attn = layer(go_feat, pro_feat_for_go, go_mask, pro_mask)
            all_go_layers_attn.append(single_go_layer_attn)

        return pro_feat_new, all_pro_layers_attn, all_go_layers_attn


class Predictor(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.seq_encoder = SeqEncoder(args.seq_dim, args.seq_hid_dim, args.seq_encoder_layer_num, args.kernel_size, args.max_pro_seq_len, dropout=args.seq_dropout)
        self.struc_encoder = StructureEncoder(args.node_in_dim, args.node_h_dim, args.edge_in_dim, args.edge_h_dim, seq_in=False, num_layers=args.struc_encoder_layer_num, drop_rate=args.struc_dropout)
        self.seq_struc_funsion = SeqStrucFusion(args.seq_struc_n_layers, args.seq_struc_n_heads, args.seq_struc_pf_dim, args.seq_hid_dim, args.struc_hid_dim, args.protein_dim, args.seq_struc_dropout)
        self.goterm_encoder = GotermEncoder(args.go_input_dim, args.go_dim, args.go_n_heads, args.go_n_layers, args.go_pf_dim, args.go_dropout)
        self.pro_go_cross_fusion = ProGoCrossFusion(args.protein_dim, args.go_dim, args.seq_struc_n_heads, args.go_n_heads, args.fusion_pro_n_layers, args.fusion_go_n_layers,
                                                        args.seq_struc_pf_dim, args.go_pf_dim, args.seq_struc_dropout, args.go_dropout)
        # self.classification_layer = ClassificationLayer(args.seq_hid_dim)
        self.device = args.device
        self.add_structure = args.add_structure
        self.add_goterm = args.add_goterm
        self.wo_msa = args.wo_msa
        self.no_go = args.no_go
        if self.wo_msa:
            self.embedding = nn.Embedding(22, 768)
    ...
    def make_masks(self, proteins_num, goterms_num, protein_max_len, goterm_max_len):
        ...
        protein_mask = protein_mask.unsqueeze(1).unsqueeze(2)  # mark
        goterm_mask = goterm_mask.unsqueeze(1).unsqueeze(2)  # mark
    ...
    def forward(self, seq_feat, struc_feat, proteins_num, goterms, goterms_num):
        if self.wo_msa:
            seq_feat = self.embedding(seq_feat).to(self.device)
        ...
        all_attn = [] # [SeqStrucFusion_attn, ProGoCrossFusion_pro_attn, ProGoCrossFusion_go_attn]
        ...
        seq_struc_emb, seq_struc_attn = self.seq_struc_funsion(seq_emb, struc_emb, proteins_mask)
        all_attn.append(seq_struc_attn)

        """
        if self.no_go:
            temp_goterms_mask = torch.zeros_like(goterms_mask)
            goterms_mask = temp_goterms_mask.to(device)
        goterms_emb = self.goterm_encoder(goterms, goterms_mask)

        pro_emb, go2pro_attn, pro2go_attn = self.pro_go_cross_fusion(seq_struc_emb, goterms_emb, proteins_mask, goterms_mask)
        all_attn.append(go2pro_attn)
        all_attn.append(pro2go_attn)
        """
        if self.no_go:
        ...
        # pro_emb降维 (seq_len, 512) -> 512
        proteins_mask = proteins_mask.squeeze(-2).squeeze(-2)
        norm = torch.norm(pro_emb, dim=2)
        norm = norm.masked_fill(proteins_mask==0, -1e10)
        norm = F.softmax(norm, dim=1)
        pro_emb = torch.bmm(pro_emb.permute(0, 2, 1), norm.unsqueeze(-1))
        pro_emb = pro_emb.squeeze(-1)
        
        return all_attn, pro_emb
    ...
    def __call__(self, data):
        ...
        return self.forward(seq_feat, struc_feat, proteins_num, goterms, goterms_num)

...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels = torch.tensor(df['labels'], dtype=torch.long) # torch.float32
...
def pack(pro_tokens, go_tokens, pro_ids, goterms, labels, dataset_path):
    ...
    pro_tokens_pad = []
    for pro_token in pro_tokens:
        pro_tokens_pad.append([pro2id_dict[t] if t in pro2id_dict else 21 for t in pro_token] + [21]*(proteins_len-len(pro_token)))
    ...
    data_pack_dict["labels"] = torch.tensor(labels)#, dtype=torch.float32
...
class ProteinGraphDataset(data.Dataset):
    ...
    def __init__(self, data_list, 
        ...
        self.letter_to_num = {'C': 4, 'D': 3, 'S': 15, 'Q': 5, 'K': 11, 'I': 9,
                       ...
                       'N': 2, 'Y': 18, 'M': 12, 'X': 20, '#': 21, 'Z': 22, 'B': 23} # 为FoldClassess新增Z, B

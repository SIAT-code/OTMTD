...
def extract_dataset_targets(d):
    ...
    if (indices is None) and (targets is not None):
        indices = np.arange(len(targets)) # for ~Subset dataset, generate indices according to the len of targets/dataset
    elif indices is None:
        indices = np.arange(len(dataset))
    else:
        indices = np.sort(indices) # for Subset dataset, directly use the corresponding indices
...
def load_full_dataset(data, targets=False, return_both_targets=False,
    ...
    orig_idxs = None
    ## 先maxsampling到maxsamples数，然后处理/装载DataLoader
